/**
 * @file
 * @brief Demonstrates how to write the main.cpp file that uses the library to integrate custom hardware modules with
 * the communication interface running on the companion host-computer (PC).
 *
 * This example uses the TestModule class implemented in the example_module.h file and is designed to be executed
 * together with the companion ataraxis-communication-interface library running on the host-computer (PC).
 */
