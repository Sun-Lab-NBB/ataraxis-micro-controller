...
class TransportLayer
{
  ...
  public:
     ...
     * received packet. The presence of this value inside the incoming byte-stream instructs the receiver to enter
     ...
     * time to each other. Primarily, this is a safeguard to break out of stale packet reception cycles and avoid
    ...
    bool SendData()
    {
        ...
        // the data to send includes the CRC checksum size. If the returned combined_size is not 0, this indicates that
        // the packet has been constructed successfully.
     ...
     * kMaximumTransmittedPayloadSize class template parameter value, the method will abort and return 0 to indicate no
     * bytes were written.
     *
     ...
     * The template uses this parameter to instantiate the appropriate version of the method for any valid object.
     ...
     * explicitly prevents reading the data outside the payload region as it may be set to valid values that
     ...
     * return 0 to indicate no bytes were read. Use get_rx_payload_size() method to get the size of the currently
     * stored payload.
     *
     * @note This method operates specifically on the region allocated for the payload of the packet. It implicitly
     * handles the necessary transformations of the start_index to make sure start_index 0 corresponds to the start
     ...
     * final index of the variable that was read by the method into the provided object. This allows using the output
     * of the method as a start_index for further read operations to ensure data contiguity. If method runtime fails,
    ...
    )
    {
        ...
        // of bytes from the start_index. Gets the current number of payload bytes from the tracker variable inside
  ...
  private:
    ...
    /// assist evaluating the behavior of the tested TransportLayer class methods.
    Stream& _port;
    ...
    /// enough space for the user-defined payload_size and all the service variables (preamble, postamble, COBS
    /// variables) that are necessary for the proper functioning of the class.
     ...
     * payload_size tracker found at index 1. After COBS encoding, the delimiter is appended to the end of the new
     * payload. Note, the packet includes the statically allocated preamble (start byte and payload size) as well as
     * the overhead byte and the delimiter byte. Finally, calculates the CRC checksum for the constructed packet
     * without the preamble (only the overhead + payload + delimiter are included in this calculation) and appends
     ...
     * (returns 0), decodes the payload using COBS to get the original payload. See CRCProcessor documentation if you
